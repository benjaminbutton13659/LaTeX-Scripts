...
$$U(q) \equiv \underbrace{U(q_0)}_{\textrm{oEdA } =\  0} + \underbrace{\frac{dU}{dq} \bigg|_{q_0}}_{\substack{ =\  0 \tx{,da}\\ \textrm{GGWlage}}} (q-q_0) + \frac{1}{2} \underbrace{ \frac{d^2 U}{d q^2} \bigg|}_{= k} (q-q_0)^2 + \dots$$
...
\subsection{ 6. gekoppelte Oszillatoren}
...
$$\underbrace{V}_{\substack{\textrm{Operator} \\ \textrm{Abbildung} \\ \textrm{Matrix}}} \cdot \underbrace{\vec{a}_i}_{\textrm{Eigenvektor}} = \underbrace{\lambda_i}_{\textrm{Eigenwert}} \cdot \underbrace{\vec{a}_i}_{\textrm{Eigenvektor}}$$
...
\subsection{ 7 Eigenschwingungen}
...
\underline{Bsp:} Normalmoden von Wasser\\
% bild 
Bilder: Wasser Molekül mit Biegeschwingung und symmetrischer oder asymmetrischer Streckschwingung
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item auch die QM kann durch ein Variationsprinzip
	\end{itemize}
\end{itemize}

% hier fehlt was folien

\paragraph{Schwingung einer Seite}
...
Damit ist
\begin{equation*}
L(\dot{u},u') = \int_{0}^{l} dx \ \ub{\bigg[ \frac{\rho}{2} \dot{u}^2 - \frac{P}{2} u'^2\bigg]}_{\tx{ \underline{Lagrange-Dichte}} \mathcal{L} (\dot{u},u') } \tag{3}
\end{equation*}
...
% hier rmbox eulergleichung in fav bild einfügen und weiter unten (8) nachtragen:
...
\begin{align*}
& \rightarrow \omega_1(t) = a \sin (\Omega t + \varphi_0)\\
\omega_2 = \frac{\dot{\omega}_1}{\Omega} \quad & \rightarrow \omega_2(t) = a \cos(\Omega t + \varphi_0)
\end{align*}
...
%\begin{align*}
%T_{\tx{rot}} &= \frac{1}{2} \sum_{n=1}^{N} m_n (\vec{\omega} \time \vec{r})^2\\
...
$$L = T-U = \ub{\frac{1}{2} m \dot{\vec{R}}^{\, 2}}_{=0} + \frac{1}{2} I_1 \omega_1^2 + \frac{1}{2} I_2 \omega_2^2 + \frac{1}{2} I_3 \omega_3^2 - mg s \cos \theta$$
Mit Eulerwinkel 
\begin{align*}
...
\begin{equation*}
L = \frac{I_1}{2} \bigg( \dot{\Phi}^2 \sin ^2 \Theta + \Theta ^2 \bigg) + \frac{I_3}{2} \bigg( \dot{\Phi} \cos \Theta + \dot{\Psi}\bigg) ^2 - mgs \cos \Theta \tag{1}
\end{equation*}
\subsubsection{\underline{Symmetrien:}}
\begin{equation*}
\frac{\partial L}{\partial t} = 0 \quad , \quad \frac{\partial L}{\partial \Theta} = 0 \quad , \quad \frac{\partial L}{\partial \Psi} = 0
\end{equation*}
...
$$ \tx{Hamilton Funktion } \quad H(q,p,t) \quad \tx{,,Hamiltonian``}$$
...
\end{equation*}
leiten wie nun $ H(q,p,t) $ von $ L(q,\dot{q},t) $ her.
...
\textbf{HIER FEHLT ALLES!!!}

mit: Bewegungsgleichungen sind äquivalent 
\begin{align*}
	\bullet & \quad \frac{d}{dt} \prt{L}{\dot q_i} = m_i \ddot q_i = \prt{L}{q_i} = - \prt{U}{q_i}\\
	\bullet & \quad \dot q_i = \prt{H}{p_i} = \frac{p_i}{m_i}\ ,\quad \dot q_i = - \prt{H}{q_i} = - \prt{U}{q_i}\ \rightarrow\ m_i \ddot q_i = - \prt{U}{q_i} \ \\
	\bullet & \quad F_i = m_i \ddot q_i = - \frac{\partial U}{\partial q_i}
\end{align*}
Mechanik nach
\begin{description}
	\item [Newton:] \
	...
	\item [Lagrange:] \
	\begin{itemize}
		\item über Def. von $L(q,\dot q, t)$
		\item Berücksichtigung von Zwangsbedingungen
		\item Zwangkräfte (Lag. Gl. 1. Art)
		\item Konzept von verallg. Koord. $q_i$
		\item Konzept von zyklischen Variabeln, $\partial L / \partial q_i = 0$
		\item[$\rightarrow$] Erhaltung von $P_i= \partial L / \partial \dot q_i$
		\item Hamilton-Prinzip
		\item Ableitung von Feld theorien 
	\end{itemize}
	\item [Hamilton:] \
	\begin{itemize}
		\item[-] über Def. von $H(q, p,t)$
		\item[-] Zwangsbedingungen nur implizit
		\item[-] Konzept des Phasenraum
		\item[$\rightarrow$] Ausgangspunkt für \underline{Statistische Mechanik}\\
		Ausgangspunkt für \underline{QM}
	\end{itemize} 
\end{description}
\section{Phasenraum}
...
\begin{itemize}
	...
	\item Zustand ist im Phasenraum eindeutig beschrieben, d.h. ( $\frac{\partial H}{\partial t}= 0$ ) schneiden sich Bahnen im Phasenraum nicht.
	\item $H(p,q) = E =$ const. $\quad$ entspricht einer 2$f - 1$ - dim. Fläche im PR, welche das $2f$-dim. \textit{Phasenraumvolumen}
\end{itemize}
\end{minipage}%
...
\begin{tikzpicture}
    	...
    	\draw (0,0) circle [radius=1.5];
   		\node [below=2cm, align=flush center,text width=4.5cm]
        {Harm. Oszill.};
\end{tikzpicture}%
\end{minipage}	
