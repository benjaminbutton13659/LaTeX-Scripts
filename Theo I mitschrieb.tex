...
\subsubsection{NG2:} $F = \dot{p} = m \cdot \vec{a}$ % tabelle
\begin{itemize}
...
\textbf{Eingesetzt:} [$\omega \pm i \omega \gamma + \omega_0^2$] $A \pm \quad = \quad f$ \quad [Ermitteln mit $z^{\star}$]
\begin{align*}
...
%\draw[fill=white, draw=none] (-3.1,0) rectangle (3.1,2);
%\draw[thick] (0,1) ellipse (3cm and 1cm);
%\draw[thick] (-3,0) --(-3,1);
%\draw[thick] (3,0) -- (3,1);
%\end{tikzpicture}
%$$
...
%\begin{tikzpicture}
%\draw [-,black] (-0.5,-1) to [out=30,in=150] (0.5,-1) to [out=-30,in=-150] (1.5,-1) to [out=90,in=90] (5.5,-1);
%\draw (1.5,-1) to [out=-90,in=-90] (5.5,-1);
%\draw[red] (3.5,-1) ellipse (2cm and .29*4cm);
%\end{tikzpicture}
...
\section{Längenkontraktion und Zeitdilatation}
\subsection{Längenkontraktion}
\begin{tikzpicture}
\draw[->] (-5,0) -- (2.5-5,0) node[right] {$x$}; 
\draw[->] (-5,0) -- (-5,2) node[above] {$y$};
\node at (0.7-5,2) [align=flush center,text width=5cm]
{IS};
\draw (-5,-.1) -- (-5,.1);
\draw (1.8-5,-.1) -- (1.8-5,.1);
\node at (-5,-.4) {$x_1$};
\node at (1.8-5,-.4) {$x_2$};
\node at (1.5-5,1) [align=flush center,text width=5cm]
{$\longrightarrow \vec{v} = v\vec{e_x}$};
\draw[->] (0,0) -- (2.5,0) node[right] {$x'$}; 
\draw[->] (0,0) -- (0,2) node[above] {$y'$};
\node at (0.7,2) [align=flush center,text width=5cm]
{IS'};
\filldraw[pattern=north east lines] (0,0) rectangle (1.8,.4);
\draw (0,-.1) -- (0,.1);
\draw (1.8,-.1) -- (1.8,.1);
\node at (0,-.4) {$x_1$};
\node at (1.8,-.4) {$x_2$};
\node[align=center] at (6,1) {Stab der \underline{Eichlänge} \\[5pt] $ l_0 = x_2' - x_1' $};
\end{tikzpicture}

% alignene und x2 x1 an stab
